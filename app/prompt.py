...
def build_system_prompt(
    ...
    if memory_context.strip():
        ...
        # 这里额外说明压缩基线 / 用户长期记忆 / 项目长期记忆 / 工作记忆的角色边界，
        # 避免模型把不同来源的内容混成一个无差别的大摘要。
        memory_header = (
            "【记忆上下文】\n"
            "以下记忆上下文按层次组织：\n"
            "1. 当前会话压缩基线：优先提供本次会话延续所需的主线事实。\n"
            "2. 固定用户长期记忆 / 相关用户长期记忆：提供稳定偏好、表达习惯和长期要求。\n"
            "3. 项目长期记忆：提供项目约束、架构约定和历史决策。\n"
            "4. 当前工作记忆：只作为本轮短期辅助，不应覆盖前面的长期信息。\n"
            "如与用户本轮明确新要求冲突，以用户本轮要求为准。\n"
        )
