...
class CuratorRunResult:
    ...
    字段说明：
    - `scanned_count`: 本次扫过多少条候选关系
    - `changed_count`: 本次实际改动了多少条记忆
    - `governed_count`: 本次直接治理了多少条目标记忆
    - `clustered_count`: 本次进入主题簇治理的种子数
    - `rewired_count`: 本次重定向了多少条旧链路
    - `stats`: 按动作类型聚合的计数
    - `changes`: 结构化变更明细
    """
...
class MemoryCurator:
    ...
    目标不是删除记忆，而是治理“当前有效版本”：
    - duplicate 归档到更强版本
    - superseded 旧版本归档出主检索面
    - 链路尽量收口到当前有效版本

    只治理 `project` scope，不碰 `user / local`。
    """
    ...
    def curate_new_entries(self, new_entries: list[MemoryEntry]) -> CuratorRunResult:
        ...
        顺序：
        1. 优先消费写入阶段已经明确给出的 `supersedes_memory_id`
        2. 再做同主题邻域治理
        3. 最后把 duplicate / supersede 链路尽量重定向到当前有效版本
        """
        ...
        for new_entry in new_entries:
            ...
            # 显式 supersede 关系优先级最高。
            # 这是写入阶段已经确认过的一条“新版本替代旧版本”信号，
            # curator 不应该再把它降回普通启发式猜测。
    ...
    def curate_project_memories(self) -> CuratorRunResult:
        """
        对当前 active project memories 做一次全量整理。

        当前仍然保持保守：
        - 只扫最近一批活跃 project 记忆作为种子
        - 不直接做全库重写式清洗
        """
    ...
    def _apply_explicit_supersede(
        ...
        """
        优先处理写入阶段显式给出的 supersede 关系。

        只要新记忆明确写了 `extra["supersedes_memory_id"]`，
        curator 就优先按这条关系治理，而不是重新猜。
        """
        ...
        if explicit_target is None or explicit_target.archived or explicit_target.id == current_entry.id:
            return current_entry
    ...
    def _decide_relation(
        ...
        same_category = self._normalize_text(candidate.category) == self._normalize_text(existing.category)
    ...
    def _rewire_memory_links(
        ...
        *,
        ...
        """
        把所有仍指向旧节点 `source_id` 的链路改写到 `target_id`。
    ...
    def _build_relation_details(
        ...
        *,
        ...
        """为 duplicate / supersede 判定补一份轻量结构化快照。"""
        details = {
    ...
    def _quality_score(self, entry: MemoryEntry) -> float:
        """给单条记忆打一个内部质量分，用于 duplicate 保留决策。"""
    ...
    def _overlap_ratio(self, left: list[str], right: list[str]) -> float:
        ...
        right_set = {self._normalize_text(item) for item in right if self._normalize_text(item)}
